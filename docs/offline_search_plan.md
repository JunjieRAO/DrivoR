# GT附近控制搜索：离线可行性实验最终方案 v2.3

2026-09-14 首轮真实运行修订：14场景因actor全生命周期条件被整场跳过、2场景因2ms时间检查报错，CEM实际候选为0。工程实现现在保留部分track，按候选检查已知区间和未知区间可达域；可达域采用显式55m/s中心速度工程上界（若观测位移更大则上调），不是数据证明的物理上界，不能作为teacher安全证书。近处未知风险仍不可核验；新增missing_actor_motion_bound_validation待验收项。评分采用官方固定帧时钟，保存原始时间抖动，半帧以上偏离/乱序仍失败。限速未知与已知超速分别报告。schema_version=2报告新增诊断与actor_replay.npz，明确搜索是否真正启动。具体实现边界及复用旧manifest/cache的重跑命令见scripts/offline_search/README.md首节。此前完整teacher验收目标不变。
